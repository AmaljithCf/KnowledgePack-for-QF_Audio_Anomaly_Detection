...
int sml_recognition_run_batch(signed short *data_batch, int batch_sz, uint8_t num_sensors, uint32_t sensor_id)
{
	...
	switch(sensor_id)
	{
		...
		case SENSOR_AUDIO_ID:
					ret = kb_run_model((SENSOR_DATA_T *)data, num_sensors, KB_MODEL_Tensorflow_model_1_INDEX);
		if (ret >= 0){
			kb_print_model_result(KB_MODEL_Tensorflow_model_1_INDEX, ret);
			sml_output_results(KB_MODEL_Tensorflow_model_1_INDEX, ret);
			kb_reset_model(0);
		};
			break;
